...
import { camera, recenter } from './camera';
  ...
  return (
    <div className="hud">
      ...
      <div className="topright">
        <button className="gbtn" title="Return to origin" onClick={() => recenter()}>
          ⌖
        </button>
        <button className="gbtn" title="Open the catalogue" onClick={() => store.openDrawer()}>
          ☰
        </button>
      </div>

      {!hintGone && <div className="hint">Drift the chart · select a world to tune in</div>}
    </div>
  );
});

...
/**
 * Button that opens a menu letting the user go to sectors as an alternative
...
export const SectorsListButton = observer(
    (props: { onClick: (nav: NavigateFunction, id: MixID) => void }) => {
        ...
        return (
            <Menu.Root>
                <Menu.Trigger
                    data-testid="button.sector-list"
                ...
                <Menu.Portal>
                    ...
                    <Menu.Positioner side="right">
                        <Menu.Popup
                            data-testid="list.sector-list"
                            className="py-4 bg-plum-500 font-medium text-white transition-all rounded-md shadow-[3px_3px_0px_black] hover:shadow-none hover:translate-x-[3px]"
                        >
                            {Mixes.map((mix, idx) => {
                                const selectedDot =
                                    store.selectedId === mix.id ? <span>X</span> : null;

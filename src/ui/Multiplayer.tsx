...
import { connectPresence, makeSelf, PresenceHandle } from '../engine/presence';
...
const Multiplayer = observer((): React.JSX.Element | null => {
    ...
    useEffect(() => {
        if (!supabase) return;
        const handle = connectPresence(supabase, makeSelf(), (peers) =>
    ...
    useEffect(() => {
        ...
        const loop = (t: number) => {
            ...
            for (const peer of store.peers) {
                ...
                if (!el) continue;
                const world = peer.listeningTo
                    ? store.worldById(peer.listeningTo)
                    : undefined;
                if (!world) {
